...
<div  className="flex flex-col items-center h-full mt-20 gap-7">
    <div className="bg-[#FFFFFF] flex flex-col justify-center gap-5  h-[50%] md:shadow-[0_0_7px_0px_gray] px-6 rounded-lg w-90">
        ...
        <div className={`cursor-text border h-13 flex flex-col justify-center p-3 rounded-md w-full ${showEmail ? "border-[#0a66c2] border-2" : "border-[silver]"}`}>
         ...
         <label onFocus={()=>{setShowPass(true)}} onBlur={()=>{if(pass === ""){setShowPass(false)}}}>

        <div className={`cursor-text relative border  h-13 flex flex-col justify-center py-3 pl-3 pr-15 rounded-md w-full ${showPass ? "border-[#0a66c2] border-2" : "border-[silver]"}`}>
            <p className={`transition-all duration-200 text-[silver] ${showPass ? "text-[14px] text-black " : "text-[18px]"}`}>Password</p>
            <input  value={pass} onChange={(e)=>{setPass(e.target.value)}} type={hidePass ? "password" : "text"} className={`outline-none transition-all duration-300 text-[20px] ${!showPass ? "h-0" : "h-8" } `}/>
            <button onClick={()=>{setHidePass(!hidePass)}} className="transition-hover duration-200 font-semibold absolute text-[#0a66c2] right-1 hover:bg-[#D0E8FF] w-13 text-center rounded-full focus:border-2 focus:bg-[#D0E8FF] focus:border-[#0a66c2]">{hidePass ? "Show" : "Hide"} </button>
        </div>
         </label>
        ...
        <p onClick={()=>navigate("/forgotpass")} className="text-[#0a66c2] rounded-full w-35 h-8 text-center pt-1 hover:underline hover:bg-[#D0E8FF] cursor-pointer">Forgot password?</p>
        <button onClick={()=>navigate("/home")} className="text-white bg-[#0a66c2] py-3 rounded-full cursor-pointer w-full hover:bg-[#004182]">Sign in</button>
    </div>
    <p className="text-center flex items-center justify-center">New to LinkedIn? <span onClick={()=>{navigate("/login")}} className="text-[#0a66c2] hover:underline cursor-pointer rounded-full w-19 h-8 flex items-center justify-center  hover:bg-[#D0E8FF]">Join now</span></p>
</div>
    </div>
  )
}
